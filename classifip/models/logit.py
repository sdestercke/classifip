import abc, math, time, random, numpy as np, scipy
...
class BinaryILogisticLasso(ImpreciseLogistic):
    ...
    def learn(self, learn_data_set=None, X=None, y=None, nb_lasso_models=20):
        ...
        self._gammas = np.linspace(start=0, stop=1, num=nb_lasso_models)
    ...
    def evaluate(self,
        ...
        for t, test in enumerate(test_dataset):
            ...
            for i, lasso_fitted in enumerate(self._lasso_models):
                ...
                _clazz = cvglmnetPredict(obj=lasso_fitted,
                                         newx=np.array([test]),
                                         s='lambda_min',
                                         ptype='class')[0]
