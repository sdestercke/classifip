...
def plot_save_results(results_dict, texte):
    if not os.path.exists('Euclidean_distance_results'):
        os.makedirs('Euclidean_distance_results')

    # save results into json file
    with open('Euclidean_distance_results/%s.json' % texte, 'w') as outfile:
    ...
    plt.grid(True)
    plt.savefig("Euclidean_distance_results/%s.png"%texte)
...
def experiment_03():
    ...
    with open('Euclidean_distance_results/summary_%s.json' % dataset_name, 'w') as outfile:
